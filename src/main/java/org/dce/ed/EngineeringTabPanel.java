...
public class EngineeringTabPanel extends JPanel {
    ...
    private JPanel buildTradePanel(Font base, int fontSize) {
        ...
        JPanel north = new JPanel(new BorderLayout());
        north.setOpaque(false);
        north.add(tradeSummaryLabel, BorderLayout.SOUTH);
        ...
        tradeScroll = wrapScroll(tradeSectionsPanel, 120);
    ...
    private void refreshUi() {
        ...
        Map<String, Integer> required = planner.requiredMaterials(goals);
        Map<String, Integer> shortfalls = planner.shortfalls(goals, inv);
        List<TradeSuggestion> trades = tradePlanner.suggest(shortfalls, inv, required);
        Map<String, Integer> invAfterTrades = tradePlanner.inventoryAfterTrades(inv, trades);
        Map<String, Integer> shortfallsAfterTrades = planner.shortfalls(goals, invAfterTrades);
        List<ShoppingListRow> shopping = planner.buildShoppingList(goals, inv, invAfterTrades);
        ...
        boolean showShopping = hasGoals && !shopping.isEmpty();
        boolean showTrades = hasGoals && !trades.isEmpty();
    ...
    private void updateTradeSections(List<TradeSuggestion> trades) {
        ...
        for (Map.Entry<String, List<TradeSuggestion>> entry : grouped.entrySet()) {
            JLabel header = traderTypeHeader(entry.getKey(), base, fontSize);
            header.setAlignmentX(Component.LEFT_ALIGNMENT);
            tradeSectionsPanel.add(header);
            for (TradeSuggestion trade : entry.getValue()) {
                JLabel line = tradeLine(trade, base, fontSize);
                line.setAlignmentX(Component.LEFT_ALIGNMENT);
                tradeSectionsPanel.add(line);
            }
    ...
    private static JLabel tradeLine(TradeSuggestion trade, Font base, int fontSize) {
        JLabel label = new JLabel(trade.summary());
        label.setFont(base.deriveFont(Font.PLAIN, fontSize));
        label.setForeground(EdoUi.Internal.MAIN_TEXT_ALPHA_220);
        label.setBorder(new EmptyBorder(1, 16, 1, 4));
        return label;
    }
    ...
    private void updateGoalsSummary(Map<String, Integer> inv, Map<String, Integer> invAfterTrades) {
        ...
        int readyNow = planner.countGoalsWithReadiness(goals, inv, invAfterTrades, GoalReadiness.READY);
        int readyWithTrades = planner.countGoalsWithReadiness(goals, inv, invAfterTrades, GoalReadiness.READY_WITH_TRADES);
        int stillShort = planner.countGoalsWithReadiness(goals, inv, invAfterTrades, GoalReadiness.STILL_SHORT);
        int readyAfterTrades = readyNow + readyWithTrades;

        StringBuilder sb = new StringBuilder();
        sb.append("<html><body style='color:#ffcc88'>");
        sb.append(readyNow).append(" of ").append(goals.size()).append(" goals ready now");
    ...
    private void updateTradeSummary(List<TradeSuggestion> trades,
        ...
        if (remainingTypes == 0) {
            tradeSummaryLabel.setText("<html><body style='color:#90ee90'>Following all "
                    + trades.size() + " trade(s) below would cover every material shortfall.</body></html>");
        } else {
            tradeSummaryLabel.setText("<html><body style='color:#ffcc88'>Following all "
                    + trades.size() + " trade(s) below still leaves "
                    + remainingTypes + " material type(s) short — collect: "
    ...
    private final class GoalsTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            return switch (columnIndex) {
                case 0 -> g.getModuleType() + ": " + g.getBlueprintName();
                case 1 -> EngineeringGradeProgress.progressLabel(g);
                case 2 -> g.getExperimentalId().isBlank() ? "—" : experimentalName(g.getExperimentalId());
                case 3 -> rowIndex < goalStatusText.size() ? goalStatusText.get(rowIndex) : "";
                default -> "";
            };
        }
